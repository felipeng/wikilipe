...
function clipboard(elem){
  ...
  var code = elem.parentElement.getElementsByTagName("code")[0].innerText;
...
function text2markdown(source,target){
  ...
  var text       = document.getElementById(source).value.replace(/]\((?!http|\?upload|data)/g,"](?p="); // and replace internal link with: ?p=
